...
const NotificationsPage = () => {
    ...
    const filteredNotifications = notifications.filter(n => {
        ...
        if (filterPage) {
            ...
            if (filterPage === 'أعضاء هيئة التدريس' && ((!text.includes('هيئة التدريس') && !text.includes('عضو') && !text.includes('أستاذ')) || text.includes('صفحة تحديد الأعباء') || text.includes('عبء التدريس') || text.includes('خصم الساعات'))) return false;
            if (filterPage === 'المقررات الدراسية' && (!text.includes('مقرر') || text.includes('صفحة تحديد الأعباء') || text.includes('تحديد عدد أسابيع') || text.includes('إلغاء تخصيص أسابيع'))) return false;
            if (filterPage === 'الخطة الدراسية' && (!text.includes('الخطة') || text.includes('صفحة تحديد الأعباء'))) return false;
            if (filterPage === 'تحديد الأعباء' && !text.includes('صفحة تحديد الأعباء') && !text.includes('تحديد الأعباء') && !text.includes('الأعباء') && !text.includes('العبء') && !text.includes('استقطاع') && !text.includes('خصم') && !text.includes('انتقاص') && !text.includes('تحديد عدد أسابيع') && !text.includes('إلغاء تخصيص أسابيع')) return false;
    ...
    const renderActionText = (text, actionBy) => {
        ...
        if (role) {
            ...
            } else if (mainText.startsWith('تم انقاص') || mainText.startsWith('تم إنقاص')) {
                // تظل كما هي بصيغة: تم انقاص عدد الساعات
            } else if (mainText.startsWith('تم ')) {
                mainText = mainText.replace('تم ', `قام بـ `);
            }
        }

        // حذف الأقواس المحيطة بالقيمة بعد كلمة بمقدار
        if (mainText.includes('بمقدار')) {
            mainText = mainText.replace(/بمقدار\s*\(([^()]+(?:\([^()]*\)[^()]*)*)\)/g, (match, p1) => `بمقدار ${p1}`);
        }

        // حذف جملة "في صفحة تحديد الأعباء"
        if (mainText.includes('في صفحة تحديد الأعباء')) {
        ...
        return parts.map((part, index) => {
            ...
            if (part === 'الجدول الرئيسي الموحد' || part === 'الجدول الرئيسي' || part === 'تحديد الأعباء') {
                return <span key={index} className="fw-bold text-success">{part}</span>;
            }
            if (part === 'نموذج 1' || part === 'نموذج 2') {
                return <span key={index} className="fw-bold text-dark">{part}</span>;
            }
            const redKeywords = ['حذف', 'مسح', 'الغاء', 'إلغاء', 'الغاء اعتماد', 'إلغاء اعتماد', 'الغاء المراجعة الاولى', 'إلغاء المراجعة الاولى', 'الغاء المراجعة الأولى', 'إلغاء المراجعة الأولى', 'الغاء المراجعة الثانية', 'إلغاء المراجعة الثانية', 'الغاء إنهاء الخطة', 'إلغاء إنهاء الخطة', 'الغاء صلاحية', 'إلغاء صلاحية', 'تم انقاص', 'تم إنقاص', 'انقاص', 'إنقاص', 'استقطاع', 'انتقاص'];
            if (redKeywords.includes(part)) {
                return <span key={index} className="text-danger fw-bold">{part}</span>;
            }
            if (['تفعيل صلاحية', 'إضافة مستخدم جديد', 'اضافة مستخدم جديد', 'تخصيص'].includes(part)) {

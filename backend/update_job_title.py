...
FROM professors 
WHERE original_workplace LIKE '%معيدة%' OR original_workplace LIKE '%معيده%'
""")
...
for row in rows:
    ...
    if job_title != 'معيدة' and job_title != 'معيد':
        cursor.execute("UPDATE professors SET job_title = 'معيدة' WHERE id = ?", (prof_id,))
